/**
 * Searchable Dropdowns for SimCard Forms
 * Provides search functionality for SimCard assignment dropdowns
 */

class SimCardSearchableDropdown {
    constructor(selectElement, options = {}) {
        this.select = selectElement;
        this.options = {
            placeholder: 'Type to search SimCards...',
            minLength: 1,
            ...options
        };
    ...
    renderOptions(options) {
        ...
        if (options.length === 0) {
            ...
            noResults.textContent = 'No SimCards found';
...
window.SimCardSearchableDropdown = SimCardSearchableDropdown;

// Initialize searchable dropdowns when DOM is loaded
document.addEventListener('DOMContentLoaded', function() {
    // Initialize for SimCard assignment dropdowns
    const simCardSelects = document.querySelectorAll('select[name="simCardId"], select[name="vehicleId"]');
    simCardSelects.forEach(select => {
        if (select.options.length > 1) { // Only initialize if there are options
            new SimCardSearchableDropdown(select);
        }
    });
}); 